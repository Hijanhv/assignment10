...
- **Severity:** `should-fix` (arguably `blocker` — it defeats a stated requirement)
...
- **Severity:** `should-fix` (maintainability judgment call)
...
- **Index as `keyExtractor`** — `BookmarksScreen.jsx` line 56 uses `String(index)`. Because rows are removed on unbookmark, index-based keys make React reconcile the wrong rows (stale images/state on recycled `FlatList` cells). Use `item.id` or `item.lessonId`. *(Borderline should-fix given the list mutates.)*
